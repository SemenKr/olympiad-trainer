...
- The Dashboard and Progress Flow assigns one immediate action to Dashboard, keeps Progress as the broader history/evidence view, and gives paused or active work precedence over a new task.
...
**Practice** is not a third permanent destination. It is the Core Practice Flow opened by Home's **Start practice**, **Resume practice**, or **Return to the current problem** action. This preserves one clear start point while keeping active work immediately recoverable.
...
| **No paused session** | There is no unfinished work to return to. | **Start practice** when available. | That the learner must resume, review, or finish something first. |
...
flowchart TD
    ...
    state -->|current problem open| returnProblem[Return to the current problem]
    state -->|session paused| resume[Resume practice]
    state -->|ready for a new task| start[Start practice]
    returnProblem --> practice[Core Practice Flow]
    resume --> practice
    start --> practice
    practice -->|pause| home
...
- **Return to the current problem** restores an active problem without creating a new attempt.
- **Resume practice** restores a paused session and its context.
- **Start practice** asks for a new useful problem only when no active or paused session takes precedence.
...
| **Practice** | Work on the active or newly selected problem. | Home's start/resume/return action. | It is a temporary task context, not a permanent top-level destination or a second recommendation system. |
...
`Home` is preferable to `Today` for the MVP because the action may be resuming a paused problem or viewing a temporarily unavailable recommendation, not a daily schedule. `Progress` is clear as a history-oriented destination. `Practice` is clear in context but does not need permanent top-level placement because Home already exposes its only MVP entry actions.

### Is a separate Practice destination necessary?

No. The Dashboard / Home action already starts, resumes, or returns to practice based on the learner's current situation. A separate permanent Practice destination would either duplicate that action or force a second decision about starting a session. It may become useful later if the product adds learner-authorised modes that genuinely change the practice experience; no such mode is in scope now.
...
| **Paused session** | Make Resume practice the primary action. | Preserve that a session is resumable when returning to Home. | Resume restores the Core Practice context. |
| **Active problem** | Make Return to the current problem the primary action. | Progress remains viewable, but must not replace or reset the active problem. | Returning restores the same problem episode. |
...
| **Home** | Resume practice | **Core Practice Flow** | Core Practice restores paused session context. |
| **Home** | Return to the current problem | **Core Practice Flow** | Core Practice restores the active problem and its evidence provenance. |
...
- There is no dead end after paused work: Home makes resumption visible and retains the problem context.
...
| **Learner opens Progress while a session is paused** | Preserve the resumable session; Home still prioritises Resume practice afterward. | That viewing history completed, abandoned, or restarted the session. |
| **Reload while on an active problem** | Core Practice restores the active episode and Home's return action remains accurate if the learner leaves the problem. | A new first attempt or erased hint/solution context. |
...
This makes Practice easy to find, but duplicates Home's situation-specific start/resume/return action. It adds navigation without a distinct student need in the current MVP.
...
3. **What should the learner land on later?** Home, because it restores the current action: return to an active problem, resume paused practice, or start a useful new session.
...
7. **Are there any dead ends across OT-002.1–002.3?** No. First visit, active/paused practice, session finish, Progress, unavailable recommendation, and pending assessment all have a documented next path or stable return state.
...
- Wireframes must test the transition language for Start, Resume, Return, Finish, and Progress without creating a separate practice mode.
...
| Navigation loses active work | Active and paused sessions retain precedence and provenance through Home and Progress. |
