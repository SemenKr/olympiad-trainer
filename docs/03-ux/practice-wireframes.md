...
| No appropriate next task | Show `Сейчас нет подходящей задачи. Можно вернуться позже.` Primary `На главную` retains the unfinished session and its context; secondary `Завершить тренировку` explicitly completes it and enters Summary. On Home, the known unavailable request uses primary `Завершить тренировку` and secondary Progress, not a Resume action that leads only back to unavailability. No substitute task or automatic finish. |
