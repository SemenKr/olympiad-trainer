...
| Home — unfinished practice | Identify the unfinished session and its latest context, including the current problem, pending assessment, or previously opened help. | `Продолжить тренировку` | Progress remains accessible. Resumption restores the episode without creating an attempt. |
...
Home's first, returning, and unfinished-practice variants share one action region. Unfinished work replaces the start action with `Продолжить тренировку`; it does not add a second primary button. The current problem remains inside Practice. First-visit and returning-history details may vary independently from task availability.
...
Following `Главная` or `Мой прогресс` from active Practice leaves the session resumable. Home then shows the single unfinished-practice state with primary `Продолжить тренировку`. Explicit `Завершить тренировку` remains a separate secondary action leading to Session summary. Navigation must not reset work or imply completion.
