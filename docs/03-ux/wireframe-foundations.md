...
| Home — unfinished practice | Identify the unfinished session and its latest context, including the current problem, pending assessment, or previously opened help. | Blocking technical failure: `Попробовать снова`; known unavailable next task: `Завершить тренировку`; otherwise, when the session can resume: `Продолжить тренировку`. | Progress remains secondary. Recovery and resumption preserve the episode; only explicit finish enters Summary. |
...
| Progress | Qualitative descriptions, their scope, recent work, and justified open questions. | `На главную` | Home restores the appropriate start/resume action. Progress provides no competing task picker. |

Home's first, returning, and unfinished-practice variants share one action region. For unfinished work, choose recovery before known task unavailability, then actual resumption; never add a second primary button. Without an unfinished session, ready Home uses `Начать тренировку`, while general task unavailability may offer Progress. The current problem remains inside Practice.
...
| Home or Practice — requesting a problem | Show `Подбираем задачу…` near the initiating action and retain session context. | The initiating primary action is busy. Pause and explicit finish remain available for an active request. |
| Home — no appropriate task available | Show `Сейчас нет подходящей задачи.` Preserve recent work and any unfinished session context. | With an unfinished session/request: primary `Завершить тренировку`, secondary Progress. Without one: primary `Посмотреть прогресс`, with return-later wording. No substitute task or automatic retry loop. |
...
| Recoverable loading error | In the affected region, use `Не удалось загрузить. Попробуй ещё раз.` Retain available content and entered work. Distinguish this from absence of suitable tasks. | Recovery becomes primary only when the main interaction is blocked; Home uses `Попробовать снова`, including for blocked unfinished work. Other existing local recovery labels remain valid. Secondary-content retry stays secondary. Never resubmit a pending answer through a loading retry. |
...
Following `Главная` or `Мой прогресс` from active Practice leaves the session unfinished. Home retains that context and uses its recovery → known unavailability → actual resumption precedence. `Завершить тренировку` is secondary when resumption or recovery is primary, and primary when no suitable next task is available. Only this explicit completion enters Session Summary; navigation never resets or completes work.
...
“Immediately visible” means visible in the relevant interaction region without opening a menu, hovering, or dismissing an overlay; it cannot mean every control is always in a 320px viewport. Home's current primary action follows its brief explanation before recent history. Answer instructions and `Проверить` stay adjacent to the answer; retry stays adjacent to the response and its feedback; pending options stay beside the pending message; continuation stays with the completed outcome. In Practice, hint, skip, eligible solution reveal, and session finish are plainly labeled secondary controls, not hidden in an overflow menu. Home instead makes Finish primary when an unfinished session has no suitable next task. Long content may require vertical scrolling.
...
7. **Which actions must remain immediately visible?** Home's start/resume action, the response's submit/retry action, pending-state options, and outcome continuation appear directly in their relevant regions. Hint, skip, eligible solution reveal, and session controls remain visible secondary actions. No requirement forces every action above a long statement's fold.
...
- Test whether learners understand `Тренировка не закончена`, `Продолжить тренировку`, `Продолжить без проверки`, and `Завершить тренировку`. Pending-state wording and the primary pause action need particular attention.
